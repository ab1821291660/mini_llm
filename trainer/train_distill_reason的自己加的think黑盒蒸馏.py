...
def train_epoch(epoch, loader, iters, tokenizer, lm_config, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        input_ids = input_ids.to(args.device)#b8-720
        labels = labels.to(args.device)#b8-720
        ...
        with autocast_ctx:#b8-720-768  #b8-720-6400
            res = model(input_ids) ##===================================



            shift_logits = res.logits[..., :-1, :].contiguous()#b8-719-6400
            shift_labels = labels[..., 1:].contiguous()#b8-719
            loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1)).view(shift_labels.size())#b8-719
            ##
            loss_mask = (shift_labels != -100).float()#b8-719
            # 特殊标签位置增加权重（推理蒸馏特有）
            sp_ids = torch.isin(shift_labels.view(-1),
                                torch.tensor(start_of_think_ids + end_of_think_ids
                                             + start_of_answer_ids + end_of_answer_ids
                                             ).to(args.device))
            loss_mask_flat = loss_mask.view(-1)#b8*719
            loss_mask_sum = loss_mask_flat.sum()#tensor(3178., device='cuda:0')##===================================##===================================
            loss_mask_flat[sp_ids] = 10
            loss_mask = loss_mask_flat.view(shift_labels.size())#b8-719
            ##
            logits_loss = (loss * loss_mask).sum() / loss_mask_sum#tensor(2.1774, device='cuda:0', grad_fn=<DivBackward0>)##===================================##===================================
...
if __name__ == "__main__":
    ...
    parser.add_argument('--from_weight', default='dpo', type=str, help="基于哪个权重训练，默认dpo,,或者full_sft")##===================================##===================================
...
#Epoch:[1/1](100/23768), loss: 1.8087, logits_loss: 1.8087, aux_loss: 0.0000, lr: 0.00000100, epoch_time: 89.0min
# Epoch:[1/1](200/23768), loss: 1.8881, logits_loss: 1.8881, aux_loss: 0.0000, lr: 0.00000100, epoch_time: 67.0min
