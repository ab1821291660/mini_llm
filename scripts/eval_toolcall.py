...
def parse_tool_calls(text):
    ...
    for m in matches:
        try:
            calls.append(json.loads(m.strip()))#{"name": "calculate_math", "arguments": {"expression": "256 * 37"}}
        except Exception:
            pass
    return calls
...
def run_case(prompt, tools, args, model=None, tokenizer=None, client=None):
    ...
    while True:
        if args.backend == 'local':
            ...
            tool_calls = parse_tool_calls(content)#dict==#{"name": "calculate_math", "arguments": {"expression": "256 * 37"}}##===================================
        else:
        ...
        } for tc in tool_calls] if args.backend == 'api'  else tool_calls#dict==#{"name": "calculate_math", "arguments": {"expression": "256 * 37"}}
        messages.append({"role": "assistant", "content": content} if args.backend == 'local'
                        else {"role": "assistant", "content": content, "tool_calls": [{"id": tc["id"], "type": "function", "function": {"name": tc["name"], "arguments": tc["arguments"]}} for tc in tool_calls]})


        for tc in tool_calls:##【#{"name": "calculate_math", "arguments": {"expression": "256 * 37"}}】
            name = tc["name"]
            arguments = tc["arguments"]
            print(f'📞 [Tool Calling]: {name} | args={arguments}')
            result = execute_tool(tc if args.backend == 'local'  else name, arguments)##===================================##{'result': '9472'}
            print(f'📞✅ [Tool Called]: {json.dumps(result, ensure_ascii=False)}')
            messages.append({"role": "tool", "content": json.dumps(result, ensure_ascii=False)} if args.backend == 'local'
                            else {"role": "tool", "content": json.dumps(result, ensure_ascii=False), "tool_call_id": tc["id"]})


def execute_tool(call,   arguments=None):#{"name": "calculate_math", "arguments": {"expression": "256 * 37"}}
    name = call.get("name", "") if isinstance(call, dict)    else call#calculate_math
    try:
        raw_args = call.get("arguments", {}) if isinstance(call, dict)    else arguments# {"expression": "256 * 37"}
...
def main():
    ...
    # parser.add_argument('--backend', default='api', choices=['local', 'api'], type=str, help="推理后端（local=本地模型，api=OpenAI兼容接口）")
    parser.add_argument('--load_from', default='../model', type=str, help="模型加载路径（model=原生torch权重，其他路径=transformers格式）")
    parser.add_argument('--save_dir', default='../out', type=str, help="模型权重目录")
    # parser.add_argument('--weight', default='full_sft', type=str, help="权重名称前缀（pretrain, full_sft, rlhf,agent      reason, ppo_actor, grpo, spo）")
    parser.add_argument('--weight', default='agent', type=str, help="权重名称前缀（pretrain, full_sft, rlhf,agent      reason, ppo_actor, grpo, spo）")
    ...
    # parser.add_argument('--api_base_url', default="http://localhost:8998/v1", type=str, help="OpenAI兼容接口的base_url")
    ...
    input_mode = 0
    cases = [{"prompt": case["prompt"], "tools": get_tools(case["tools"]), "tool_names": case["tools"]} for case in TEST_CASES] if input_mode == 0 \
        else iter(lambda: {"prompt": input('💬: '), "tools": TOOLS, "tool_names": [t["function"]["name"] for t in TOOLS]},    {"prompt": "", "tools": TOOLS, "tool_names": []})
    for case in cases:
